...
class DetailedSpendingPage extends StatefulWidget {
  GastoMensal month;
  DetailedSpendingPage({Key? key, required this.month}) : super(key: key);
...
class _DetailedSpendingState extends State<DetailedSpendingPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.month.mes.nome),
      ),
      body: Center(
        child: Text('Detailed Spending for ${widget.month.mes.nome}'),
      ),
    );
  }
}
          ...
          return ListTile(
            ...
            onTap: () {
              Navigator.push(
                ...
                MaterialPageRoute(
                    builder: (context) =>
                        DetailedSpendingPage(month: tabela[linha])),
              );
            },
          );
        },
